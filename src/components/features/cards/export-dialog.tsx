...
export function ExportDialog({ open, onOpenChange }: ExportDialogProps) {
  ...
  const exportCards = useMemo(() => {
    ...
    return cards;
  }, [scope, cards, selectedCardIds, filteredCards]);
  ...
  const { cardAspectRatio, nativeCardWidth } = useMemo(() => {
    ...
    return { cardAspectRatio: h / w, nativeCardWidth: w };
  }, [resolvedPreview]);

  const pdfLayout = useMemo(
    () =>
      computePdfLayout(
        { pageSize, cardsPerRow, cardGap: PDF_GAP, pageMargin: PDF_MARGIN, maintainCardSize },
        cardAspectRatio,
        nativeCardWidth
      ),
    [pageSize, cardsPerRow, maintainCardSize, cardAspectRatio, nativeCardWidth]
  );
  ...
  const handleExport = async () => {
    ...
    if (resolved.length === 0) {
      toast.error("No cards have a matching layout");
      setProgress({ status: "idle", current: 0, total: 0, message: "" });
      return;
    }

    const skipped = exportCards.length - resolved.length;
    ...
    for (let i = 0; i < resolved.length; i++) {
      ...
      try {
        const dataUrl = await renderCardToPng(
          renderRef.current,
          resolved[i].layout.width ?? DEFAULT_CARD_WIDTH,
          resolved[i].layout.height ?? DEFAULT_CARD_HEIGHT,
          2
        );
        renderedImages.push({
          dataUrl,
          width: resolved[i].layout.width ?? DEFAULT_CARD_WIDTH,
          height: resolved[i].layout.height ?? DEFAULT_CARD_HEIGHT,
        });
      } catch (err) {
    ...
    setProgress({
      ...
      total: resolved.length,
      message: format === "pdf" ? "Generating PDF…" : "Preparing downloads…",
    });
    ...
    if (format === "pdf") {
      ...
      for (let i = 0; i < renderedImages.length; i++) {
        ...
        downloadPng(
          renderedImages[i].dataUrl,
          `${projectName}-${cardName}.png`
        );
    ...
    const msg =
      `Exported ${renderedImages.length} cards as ${format.toUpperCase()}` +
      (skipped > 0
        ? ` (${skipped} skipped — no matching layout)`
        : "");
    toast.success(msg);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-4xl max-h-[85vh] flex flex-col overflow-hidden">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>
            Render cards as images or a print-ready PDF.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-1 gap-6 overflow-hidden min-h-0">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="pdf">PDF Print Sheet</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col overflow-hidden rounded-lg border bg-muted/30">
            {resolvedPreview.length === 0 ? (
              ...
              <PngPreview resolved={resolvedPreview} />
            )}
          </div>
        </div>
            ...
            <>
              ...
              {progress.status !== "complete" && (
                <Button
                  ...
                  disabled={resolvedPreview.length === 0}
                >
                  Export {resolvedPreview.length} card
                  {resolvedPreview.length !== 1 ? "s" : ""}
                </Button>
              )}
            </>
          )}
...
// ─── PNG Preview: scrollable grid of card thumbnails ────────────────────────

function PngPreview({ resolved }: { resolved: ResolvedCard[] }) {
    ...
    <div className="flex-1 overflow-y-auto p-4">
      <div className="flex flex-wrap gap-3">
        {resolved.map((rc, i) => {
          ...
          return (
            <div key={rc.card.id} className="flex flex-col items-center gap-1">
              <div
      ...
      >
        ...
        >
          ...
          {pageCards.map((rc, i) => {
            ...
            const x = PDF_MARGIN + col * (cardW + PDF_GAP);
            const y = PDF_MARGIN + row * (cardH + PDF_GAP);
